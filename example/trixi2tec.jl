...
function trixi2tec(u, semi, filename; title = basename(filename),
    ...
    if ndims == 1
        ...
        # For 1D: count elements manually
        element_count = 0
        for element in eachelement(solver, cache)
            element_count += 1
        end
        total_points = ndofs_x * element_count
        zone_info = "ZONE I=$total_points, F=POINT\n"
    elseif ndims == 2
        variables = ["x", "y"]
        ndofs_x = size(u, 2)
        ndofs_y = size(u, 3)
    ...
    open(filename, "w") do io
        ...
        if ndims == 1
            # For 1D: Write a single zone with all elements combined
            write(io, zone_info)
            for element in eachelement(solver, cache)
                for ci in indices
            ...
            # For 2D and 3D: Keep the original behavior (one zone per element)
            for element in eachelement(solver, cache)
                write(io, zone_info)
                for ci in indices
                    node_coords = Trixi.get_node_coords(node_coordinates, equations, solver, ci,
