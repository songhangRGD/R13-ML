...
The Training_Ar folder contains original data from DSMC and normalized data used for machine learning. The DSMC
setting files are named as in.shockvss. All files named _input.dat are normalized data, while tmp_flowvss. files are the original data generated from DSMC.
The normalization and input-data preparation should be performed sequentially using the following commands:
python3 sort.py
python3 input.py
After the normalized input data have been generated, the neural-network model can be trained in Julia by running:
include("ML_model.jl")
The script sort.py first organizes the DSMC-derived data into the required order, while input.py performs the normalization and prepares the input/output arrays used for training. The Julia script ML_model.jl then trains the neural-network closure model using the prepared normalized data. After training, the resulting models are used within the TrixiMomentEquations framework and moved to the folder "TrixiMomentEquations\src\Machine_learning_data".
